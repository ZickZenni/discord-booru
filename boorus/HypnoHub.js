...
module.exports = class HypnoHub extends BaseBooru {
    constructor() {
        super("hypnohub", true, false, ["pussy", "pussy_juice", "masturbation"]);
    }
    ...
    async getPosts(tags, rating, limit) {
        ...
        return new Promise(( resolve, reject ) => {
            ...
            fetch(`https://hypnohub.net/post/index.xml?tags=${rating}+${tags}&limit=${limit}`)
            .then( function(res) {
                try {
                    res = res.text();
                } catch (e) {
            ...
            .then(async function(almostJSON) {
                await parseString(almostJSON, async function(err, res) {
                    ...
                    posts.forEach(element => {
                        ...
                        var json = `{ "id":"${element['$'].id}", "tags":"${jsonTags}", "author":"${element['$'].creator_id}", "compressed_file_url":"${encodeURI(element['$'].sample_url)}", "original_file_url":"${encodeURI(element['$'].file_url)}", "rating":"${element['$'].rating}", "booru":"hypnohub"}`
