...
module.exports = class SafeBooru extends BaseBooru {
    ...
    async getPosts(tags, rating, limit) {
        ...
        return new Promise(( resolve, reject ) => {
            ...
            fetch(`https://safebooru.org/index.php?page=dapi&s=post&q=index&tags=${tags}&limit=${limit}`)
            .then( function(res) {
                try {
                    res = res.text();
                } catch (e) {
            ...
            .then(async function(almostJSON) {
                await parseString(almostJSON, async function(err, res) {
                    ...
                    posts.forEach(element => {
                        ...
                        var json = `{ "id":"${element['$'].id}", "tags":"${jsonTags}", "author":"${element['$'].creator_id}", "compressed_file_url":"${element['$'].sample_url}", "original_file_url":"${element['$'].file_url}", "rating":"${element['$'].rating}", "booru":"safebooru"}`
